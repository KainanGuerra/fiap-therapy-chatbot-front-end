...
export function ChatSection({ onScheduleRequest }: ChatSectionProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      content: "Hello! I'm here to support you. How are you feeling today? You can share what's on your mind, and I'll do my best to help.",
      sender: 'bot',
      timestamp: new Date(),
    }
  ]);
  ...
  const analyzeMessage = (message: string): { needsProfessionalHelp: boolean; response: string; suggestions?: string[]; showCrisisCard?: boolean } => {
    const lowerMessage = message.toLowerCase();
    
    // Crisis indicators
    const crisisKeywords = ['suicide', 'kill myself', 'end it all', 'not worth living', 'hurt myself', 'self harm'];
    const deathKeywords = ['death', 'die', 'dying', 'want to die', 'better off dead'];
    const severeKeywords = ['severe depression', 'panic attacks', 'can\'t function', 'completely overwhelmed', 'breakdown', 'trauma', 'ptsd'];
    const chronicKeywords = ['months of', 'years of', 'always feel', 'never feel better', 'getting worse'];
    const psychologyKeywords = ['behavior patterns', 'cognitive', 'thinking patterns', 'phobia', 'ocd', 'adhd', 'autism', 'personality disorder'];
    const therapyKeywords = ['relationship', 'family', 'couples', 'grief', 'loss', 'addiction'];
    
    // Mild stress/advice keywords
    const mildKeywords = ['stressed', 'tired', 'overwhelmed', 'anxious about work', 'sleep problems', 'work stress'];
    ...
    if (crisisKeywords.some(keyword => lowerMessage.includes(keyword))) {
      return {
        ...
        suggestions: ['Find emergency support', 'Schedule urgent appointment', 'Crisis helpline numbers'],
        showCrisisCard: true
      };
    }
    ...
    if (deathKeywords.some(keyword => lowerMessage.includes(keyword))) {
      return {
        needsProfessionalHelp: true,
        response: "I'm deeply concerned about what you're sharing. Thoughts about death are serious and you don't have to face this alone. There are mental health professionals who specialize in helping people through these difficult times. Please consider reaching out for immediate support.",
        suggestions: ['Call crisis specialist', 'Schedule emergency appointment', 'Access crisis resources'],
        showCrisisCard: true
      };
    }
    ...
    if (psychologyKeywords.some(keyword => lowerMessage.includes(keyword))) {
      return {
        ...
        suggestions: ['Find a psychologist', 'Learn about CBT', 'Schedule psychological assessment']
      };
    }
    ...
    if (severeKeywords.some(keyword => lowerMessage.includes(keyword)) || 
      ...
      return {
        ...
        suggestions: ['Find a psychologist', 'Find a therapist', 'Learn about treatments', 'Schedule consultation']
      };
    }
    ...
    if (mildKeywords.some(keyword => lowerMessage.includes(keyword))) {
      return {
        ...
        response: "It sounds like you're dealing with some stress. Here are some strategies that might help: Practice deep breathing exercises, try the 5-4-3-2-1 grounding technique, maintain a regular sleep schedule, and engage in physical activity. If these feelings persist or worsen, consider talking to a psychologist or therapist who can provide personalized coping strategies.",
        suggestions: ['Try breathing exercises', 'Practice mindfulness', 'Improve sleep hygiene', 'Consider seeing a psychologist']
      };
    }
    ...
    return {
      needsProfessionalHelp: false,
      response: "Thank you for sharing with me. I'm here to listen and support you. Can you tell me more about what's been on your mind lately? Understanding your situation better will help me provide more personalized guidance. If you feel like you need more support, both psychologists and therapists can provide professional help tailored to your specific needs.",
      suggestions: ['Tell me more', 'Explore coping strategies', 'Learn about psychology', 'Find mental health support']
    };
  };
  ...
  const handleSuggestionClick = (suggestion: string) => {
    if (suggestion.includes('Find') || suggestion.includes('Schedule') || suggestion.includes('Call')) {
      onScheduleRequest();
    } else {
                ...
                {message.suggestions && (
                  <div className="flex flex-wrap gap-2">
                    {message.suggestions.map((suggestion, index) => (
                      ...
                      >
                        {suggestion.includes('Find') || suggestion.includes('Schedule') || suggestion.includes('Call') ? (
                          <Calendar className="w-3 h-3 mr-1" />
                        ) : suggestion.includes('coping') || suggestion.includes('strategies') ? (
